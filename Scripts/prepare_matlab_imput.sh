...
do
    ...
    if [ -n "$(grep $accnum ./Alignments/Final_Alignments.fasta)" ]; then
        cat ./Alignments/Final_Alignments.fasta | grep -A1 $accnum | head -n 2 | grep -v '>' | sed 's/A/1 /g' | sed 's/C/2 /g' | sed 's/G/3 /g' | sed 's/T/4 /g' | sed 's/-/N/g' | sed 's/[A-Z]/0 /g' >> ./Alignments/Matlab_fasta/${sp_name}.fasta
