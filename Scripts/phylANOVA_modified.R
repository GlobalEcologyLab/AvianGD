## Modified function of phylANOVA in the 'phytools' package. The script was taken from the git repository of the package.

phylANOVA_modified<-function(tree,x,y,nsim=1000,posthoc=TRUE,p.adj="holm"){
  if(is.null(names(x))){
  ...
  if(posthoc){
    ...
    obj<-list(F=F.obs,Pf=P.F,T=T.obs,method=p.adj,Pt=P.T,
              "Sum Sq"=aov$"Sum Sq","Mean Sq"=aov$"Mean Sq")
                   ...
                   "Mean Sq"=aov$"Mean Sq")
  class(obj)<-"phylANOVA"
  return(list(sig2, sims, F.null, obj))
}
